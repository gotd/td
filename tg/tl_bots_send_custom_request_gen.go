...
func (s *BotsSendCustomRequestRequest) Encode(b *bin.Buffer) error {
	if s == nil {
		return fmt.Errorf("can't encode %s as nil", "bots.sendCustomRequest#aa2769ed")
	}
...
func (s *BotsSendCustomRequestRequest) EncodeBare(b *bin.Buffer) error {
	...
	if err := s.Params.Encode(b); err != nil {
		return fmt.Errorf("unable to encode %s: field %s: %w", "bots.sendCustomRequest#aa2769ed", "params", err)
	}
	return nil
}
...
func (s *BotsSendCustomRequestRequest) Decode(b *bin.Buffer) error {
	if s == nil {
		return fmt.Errorf("can't decode %s to nil", "bots.sendCustomRequest#aa2769ed")
	}
	if err := b.ConsumeID(BotsSendCustomRequestRequestTypeID); err != nil {
		return fmt.Errorf("unable to decode %s: %w", "bots.sendCustomRequest#aa2769ed", err)
	}
...
func (s *BotsSendCustomRequestRequest) DecodeBare(b *bin.Buffer) error {
	...
	}
	{
		value, err := b.String()
		if err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "bots.sendCustomRequest#aa2769ed", "custom_method", err)
		}
		s.CustomMethod = value
	}
	{
		if err := s.Params.Decode(b); err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "bots.sendCustomRequest#aa2769ed", "params", err)
		}
	}
	return nil
}
