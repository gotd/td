...
func (a *MessagesArchivedStickers) Encode(b *bin.Buffer) error {
	if a == nil {
		return fmt.Errorf("can't encode %s as nil", "messages.archivedStickers#4fcba9c8")
	}
...
func (a *MessagesArchivedStickers) EncodeBare(b *bin.Buffer) error {
	...
	for idx, v := range a.Sets {
		if v == nil {
			return fmt.Errorf("unable to encode %s: field %s element with index %d is nil", "messages.archivedStickers#4fcba9c8", "sets", idx)
		}
		if err := v.Encode(b); err != nil {
			return fmt.Errorf("unable to encode %s: field %s element with index %d: %w", "messages.archivedStickers#4fcba9c8", "sets", idx, err)
		}
	}
	return nil
}
...
func (a *MessagesArchivedStickers) Decode(b *bin.Buffer) error {
	if a == nil {
		return fmt.Errorf("can't decode %s to nil", "messages.archivedStickers#4fcba9c8")
	}
	if err := b.ConsumeID(MessagesArchivedStickersTypeID); err != nil {
		return fmt.Errorf("unable to decode %s: %w", "messages.archivedStickers#4fcba9c8", err)
	}
...
func (a *MessagesArchivedStickers) DecodeBare(b *bin.Buffer) error {
	...
	}
	{
		value, err := b.Int()
		if err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "messages.archivedStickers#4fcba9c8", "count", err)
		}
		a.Count = value
	}
	{
		...
		if err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "messages.archivedStickers#4fcba9c8", "sets", err)
		}
