...
func (s *AccountSetAccountTTLRequest) Encode(b *bin.Buffer) error {
	if s == nil {
		return fmt.Errorf("can't encode %s as nil", "account.setAccountTTL#2442485e")
	}
...
func (s *AccountSetAccountTTLRequest) EncodeBare(b *bin.Buffer) error {
	...
	if err := s.TTL.Encode(b); err != nil {
		return fmt.Errorf("unable to encode %s: field %s: %w", "account.setAccountTTL#2442485e", "ttl", err)
	}
	return nil
}
...
func (s *AccountSetAccountTTLRequest) Decode(b *bin.Buffer) error {
	if s == nil {
		return fmt.Errorf("can't decode %s to nil", "account.setAccountTTL#2442485e")
	}
	if err := b.ConsumeID(AccountSetAccountTTLRequestTypeID); err != nil {
		return fmt.Errorf("unable to decode %s: %w", "account.setAccountTTL#2442485e", err)
	}
...
func (s *AccountSetAccountTTLRequest) DecodeBare(b *bin.Buffer) error {
	...
	}
	{
		if err := s.TTL.Decode(b); err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "account.setAccountTTL#2442485e", "ttl", err)
		}
	}
	return nil
}
