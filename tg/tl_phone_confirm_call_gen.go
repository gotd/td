...
func (c *PhoneConfirmCallRequest) Encode(b *bin.Buffer) error {
	if c == nil {
		return fmt.Errorf("can't encode %s as nil", "phone.confirmCall#2efe1722")
	}
...
func (c *PhoneConfirmCallRequest) EncodeBare(b *bin.Buffer) error {
	...
	if err := c.Peer.Encode(b); err != nil {
		return fmt.Errorf("unable to encode %s: field %s: %w", "phone.confirmCall#2efe1722", "peer", err)
	}
	b.PutBytes(c.GA)
	b.PutLong(c.KeyFingerprint)
	if err := c.Protocol.Encode(b); err != nil {
		return fmt.Errorf("unable to encode %s: field %s: %w", "phone.confirmCall#2efe1722", "protocol", err)
	}
	return nil
}
...
func (c *PhoneConfirmCallRequest) Decode(b *bin.Buffer) error {
	if c == nil {
		return fmt.Errorf("can't decode %s to nil", "phone.confirmCall#2efe1722")
	}
	if err := b.ConsumeID(PhoneConfirmCallRequestTypeID); err != nil {
		return fmt.Errorf("unable to decode %s: %w", "phone.confirmCall#2efe1722", err)
	}
...
func (c *PhoneConfirmCallRequest) DecodeBare(b *bin.Buffer) error {
	...
	}
	{
		if err := c.Peer.Decode(b); err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "phone.confirmCall#2efe1722", "peer", err)
		}
	}
	{
		value, err := b.Bytes()
		if err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "phone.confirmCall#2efe1722", "g_a", err)
		}
		c.GA = value
	}
	{
		value, err := b.Long()
		if err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "phone.confirmCall#2efe1722", "key_fingerprint", err)
		}
		c.KeyFingerprint = value
	}
	{
		if err := c.Protocol.Decode(b); err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "phone.confirmCall#2efe1722", "protocol", err)
		}
	}
	return nil
}
