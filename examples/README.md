...
| [bot-bigbuckbunny](bot-bigbuckbunny/main.go) | Bot replying to any message with a video                  | `client.Pool(N)` multi-connection upload pool, `uploader.WithThreads`/`WithProgress`, temp-file caching, video upload |
| [bg-run](bg-run/main.go)                   | Using client without Run                                    | contrib/bg package                                                                                         |
| [account-banned](account-banned/main.go)   | Detect banned/deactivated account vs dead proxy             | `OnSelfError` hook, `auth.IsUnauthorized` (401), banned flag, permanent reconnection stop                  |
