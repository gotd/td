...
func (p *PingRequest) Encode(b *bin.Buffer) error {
	if p == nil {
		return fmt.Errorf("can't encode %s as nil", "ping#ce73048f")
	}
...
func (p *PingRequest) Decode(b *bin.Buffer) error {
	if p == nil {
		return fmt.Errorf("can't decode %s to nil", "ping#ce73048f")
	}
	if err := b.ConsumeID(PingRequestTypeID); err != nil {
		return fmt.Errorf("unable to decode %s: %w", "ping#ce73048f", err)
	}
...
func (p *PingRequest) DecodeBare(b *bin.Buffer) error {
	...
	}
	{
		value, err := b.Int32()
		if err != nil {
			return fmt.Errorf("unable to decode %s: field %s: %w", "ping#ce73048f", "id", err)
		}
		p.ID = value
	}
	return nil
